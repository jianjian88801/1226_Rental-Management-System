<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="租客账号" prop="zukezhanghao">
          <el-select  @change="zukezhanghaoChange" v-model="ruleForm.zukezhanghao" placeholder="请选择租客账号">
            <el-option
                v-for="(item,index) in zukezhanghaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.zukezhanghao" label="租客账号" prop="zukezhanghao">
...
export default {
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.wuzhuzhanghao!=''&&json.wuzhuzhanghao){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/zuke/zukezhanghao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.zukezhanghaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    zukezhanghaoChange () {
      this.$http({
        url: `follow/zuke/zukezhanghao?columnValue=`+ this.ruleForm.zukezhanghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
